...
def plot_results(episode_rewards, episode_losses, save_path, moving_avg_window=100):
    ...
    ax1.set_title("N-Step DQN: Training Rewards")
    ...
    ax2.set_title("N-Step DQN: Training Loss")
...
def deploy(env_name, agent, num_trials=5, fps=30):
    ...
    """
    print("\nDeploying N-Step DQN Agent (matplotlib)...")
    try:
    ...
    for trial in range(num_trials):
        ...
        while not terminal:
            # Pure exploitation: no epsilon-greedy, always pick the best action
            ...
            ax.set_title(f"N-Step DQN | Trial {trial + 1} | Reward: {total_reward}")
            fig.canvas.draw()
